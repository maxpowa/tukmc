    ...
    public void run() {
        if (updchk) {
    		try {
    			...
    			mod_TukMC.updateVersion = data.substring(data.indexOf("[version]") + 9, data.indexOf("[/version]"));
    			mod_TukMC.updateText = data.substring(data.indexOf("[changes]") + 9, data.indexOf("[/changes]"));
    			mod_TukMC.updateMCVersion = data.substring(data.indexOf("[mcversion]") + 11, data.indexOf("[/mcversion]"));
    			System.out.println("[TukMC] Found v" + mod_TukMC.updateVersion + ". Running v" + mod_TukMC.TK_VERSION + ". Check completed in " + (System.currentTimeMillis() - time) + "ms.");
    			updchk = false;
    			return;
    		} catch (Exception e) {
