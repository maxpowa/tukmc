...
public final class McMMOIntegration {
	...
	public static ArrayListMultimap<String, String> toolSkillsMap = ArrayListMultimap.create();
	public static Map<String, String> skillToolsMap = new HashMap();

	public static List<SkillData> skillData = new ArrayList();
	...
	public static boolean passMessage(String s) {
		if (s.startsWith(ColorCode.GREY + "**YOU LOWER YOUR")) {
			String tool = s.substring((ColorCode.BRIGHT_GREEN + "**YOU LOWER YOUR ").length(), s.length() - 2);
			for (SkillData data : skillData)
				if (data.type == UsageType.READY && data.tool.equalsIgnoreCase(tool)) data.remove = true;
					return true;
		}
		if (s.startsWith(ColorCode.RED + "**") && s.endsWith("has worn off**")) {
			String ability = s.substring((ColorCode.RED + "**").length(), s.length() - "has worn off**".length() - 1);
			for (SkillData data : skillData)
				if (data.type == UsageType.ACTIVE && data.name.equalsIgnoreCase(ability)) data.type = UsageType.COOLDOWN;
					return true;
		}
		if (s.startsWith(ColorCode.BRIGHT_GREEN + "Your " + ColorCode.YELLOW) && s.endsWith("ability is refreshed!")) {
			String skill = s.substring((ColorCode.BRIGHT_GREEN + "Your " + ColorCode.YELLOW).length(), s.length() - (ColorCode.BRIGHT_GREEN + "ability is refreshed!").length());
			System.out.println("parsed: *" + skill + "*");
			for (SkillData data : skillData)
				if (data.type == UsageType.COOLDOWN && data.getName().equalsIgnoreCase(skill)) data.remove = true;
					return true;
		}
	...
	public static void tick() {
		if (CommonUtils.getMc().currentScreen != null && CommonUtils.getMc().currentScreen instanceof GuiMainMenu) {
			skillData.clear();
			time = 0;
		}
		if (time > 0) --time;
		List<SkillData> removeThis = new ArrayList();
		for (SkillData skill : skillData)
			if (skill.remove) removeThis.add(skill);
	...
	public static class SkillData {
		...
		public static SkillData fromString(String s) {
			if (s.startsWith(ColorCode.BRIGHT_GREEN + "**YOU READY YOUR")) {
				String tool = s.substring((ColorCode.BRIGHT_GREEN + "**YOU READY YOUR ").length(), s.length() - 2);
				return new SkillData(tool, toolSkillsMap.get(tool).get(0), UsageType.READY);
			}
			if (s.startsWith(ColorCode.BRIGHT_GREEN + "**") && s.endsWith(" ACTIVATED**")) {
				String skill = s.substring((ColorCode.BRIGHT_GREEN + "**").length(), s.length() - "ACTIVATED**".length() - 1);
				for (String skill1 : skillToolsMap.keySet())
					if (skill1.equalsIgnoreCase(skill)) return new SkillData(skillToolsMap.get(skill1), skill1, UsageType.ACTIVE);
			}
